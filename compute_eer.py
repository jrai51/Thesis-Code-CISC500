...
parser.add_argument("input_csv", help="Path to input CSV file containing columns: gen_user, test_user, window_idx, anomaly_percentage")
...
required_cols = {'gen_user', 'test_user', 'window_idx', 'anomaly_percentage'}
if not required_cols.issubset(df.columns):
    print("Input CSV is missing one of the required columns: gen_user, test_user, window_idx, anomaly_percentage")
    exit(1)
...
for user, group in df.groupby('gen_user'):
    ...
    genuine_scores = group[group['test_user'] == user]['anomaly_percentage'].values
    impostor_scores = group[group['test_user'] != user]['anomaly_percentage'].values
    ...
    # Note: We consider "accept" if anomaly_percentage <= threshold (assuming lower scores indicate more likely genuine).
